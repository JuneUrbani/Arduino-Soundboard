...
namespace Soundboard
{
    ...
    {
        static SoundProgram prog1;
        static string[] fixedData = { "0", "0", "0", "0"};
        ...
        private static void DataReceivedHandler(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            try
            {
                volume = ((maxAnalogValue - float.Parse(fixedData[1])) / maxAnalogValue) * 100f;
                volume = (float)Math.Floor(volume);

                mute = int.Parse(fixedData[0]) == 0;
            ...
            VolumeMixer.SetApplicationVolume(prog1.pID, volume);
            VolumeMixer.SetApplicationMute(prog1.pID, mute);
        }

        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            prog1 = comboBox1.SelectedItem as SoundProgram;
            VolumeMixer.SetApplicationVolume(16356, 50f);
        }
        ...
        public void populateBox(ComboBox comboBox)
        {
            ...
            foreach (AudioSession session in AudioUtilities.GetAllSessions())
            {
                if (session.Process != null)
                {
                    ...
                    dataSource.Add(new SoundProgram() { Name = session.Process.ProcessName, pID = session.Process.Id });
                }
            }
